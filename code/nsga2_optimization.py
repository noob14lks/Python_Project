...
def guided_mutation(individual, importance_scores=None, mutpb=0.1):
    ...
    return (individual,)   # Ensure mutation returns a tuple!
...
def evaluate(individual, X, y, classifier=RandomForestClassifier(n_estimators=30, n_jobs=-1)):
    ...
    if len(selected_idx) == 0:
        return (0.0, len(individual))   # Always return a tuple!
    ...
    try:
        ...
        accuracy = 0.0
    return (accuracy, len(selected_idx))   # Always return a tuple!
...
def run_nsga2(X, y, population_size=20, ngen=10, initial_pop=None, importance_scores=None):
    ...
    # Correct initialization! All individuals must have fitness assigned before selNSGA2
